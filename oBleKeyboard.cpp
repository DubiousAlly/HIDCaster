...
void BleKeyboard::click(uint8_t b)
{
  _buttons = b;
  move(0,0,0,0);
  _buttons = 0;
...
void BleKeyboard::bpress(uint8_t b)
{
  buttons(_buttons | b);
}

void BleKeyboard::brelease(uint8_t b)
{
  buttons(_buttons & ~b);
}

bool BleKeyboard::isPressed(uint8_t b)
{
  if ((b & _buttons) > 0)
    return true;
  return false;
...
void BleKeyboard::sendReport(MouseReport* MouseVals)
{
  if (this->isConnected())
  {
    this->inputMouse->setValue((uint8_t*)MouseVals, sizeof(MouseReport));
    this->inputMouse->notify();
#if defined(USE_NIMBLE)        
    //vTaskDelay(delayTicks);
    this->delay_ms(_delay_ms);
